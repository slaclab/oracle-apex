  ...
  SELECT t.element_id,
     ...
     FROM (SELECT l.element_id,
		  ...
		  COALESCE(l.engineering_name, sx.symbols_engineering_name) as engineering_name,
                  ...
                  CASE WHEN ( 'MARK' IN (SX.XAL_KEYWORD, TX.LTU_XAL_KEYWORD) ) THEN L.NONMAD_SUML_M  -- COALESCE (L.NONMAD_SUML_M, SX.SUML)
                            ELSE COALESCE (SX.SUML, L.NONMAD_SUML_M)
                  END AS SUML_M,
                  ----------
                  CASE WHEN ( 'MARK' IN (SX.XAL_KEYWORD, TX.LTU_XAL_KEYWORD) ) THEN ROUND (L.NONMAD_SUML_M * 3.28083989501, 6)  -- ROUND (COALESCE (L.NONMAD_SUML_M, SX.SUML) * 3.28083989501, 6)
                            ELSE ROUND (COALESCE (SX.SUML, L.NONMAD_SUML_M) * 3.28083989501, 6)
                   END AS SUML_FT,
                   ----------
                  CASE WHEN ( 'MARK' IN (SX.XAL_KEYWORD, TX.LTU_XAL_KEYWORD) ) THEN L.NONMAD_SOLID_EDGE_XCOOR_M  -- COALESCE (L.NONMAD_SOLID_EDGE_XCOOR_M, SX.SOLID_EDGE_X_COOR)
                           ELSE COALESCE (SX.SOLID_EDGE_X_COOR, L.NONMAD_SOLID_EDGE_XCOOR_M)
                  END AS LINACZ_M,
                  ----------
                  CASE WHEN ( 'MARK' IN (SX.XAL_KEYWORD, TX.LTU_XAL_KEYWORD) ) THEN ROUND (L.NONMAD_SOLID_EDGE_XCOOR_M * 3.28083989501, 6)  -- ROUND (COALESCE (L.NONMAD_SOLID_EDGE_XCOOR_M, SX.SOLID_EDGE_X_COOR) * 3.28083989501, 6)
                  ...
                  CASE WHEN ( 'MARK' IN (SX.XAL_KEYWORD, TX.LTU_XAL_KEYWORD) ) THEN L.NONMAD_LTU_SUML_M  --COALESCE (L.NONMAD_LTU_SUML_M, TX.LTU_SUML)
                           ELSE COALESCE (TX.LTU_SUML, L.NONMAD_LTU_SUML_M)
                  END AS LTU_SUML_M,
                  ---------
                  CASE WHEN ( 'MARK' IN (SX.XAL_KEYWORD, TX.LTU_XAL_KEYWORD) ) THEN ROUND (L.NONMAD_LTU_SUML_M * 3.28083989501, 6)  -- ROUND (COALESCE (L.NONMAD_LTU_SUML_M, TX.LTU_SUML) * 3.28083989501, 6)
                            ELSE ROUND (COALESCE (TX.LTU_SUML, L.NONMAD_LTU_SUML_M) * 3.28083989501, 6)
                  END AS LTU_SUML_FT,
                  ---------
                  CASE WHEN ( 'MARK' IN (SX.XAL_KEYWORD, TX.LTU_XAL_KEYWORD) ) THEN L.NONMAD_LTU_SOLID_EDGE_XCOOR_M  -- COALESCE (L.NONMAD_LTU_SOLID_EDGE_XCOOR_M, TX.LTU_SOLID_EDGE_X_COOR)
                           ELSE COALESCE (TX.LTU_SOLID_EDGE_X_COOR, L.NONMAD_LTU_SOLID_EDGE_XCOOR_M)
                  END AS LTU_LINACZ_M,
                  ---------
                  CASE WHEN ( 'MARK' IN (SX.XAL_KEYWORD, TX.LTU_XAL_KEYWORD) ) THEN ROUND (L.NONMAD_LTU_SOLID_EDGE_XCOOR_M * 3.28083989501, 6)  -- ROUND (COALESCE (L.NONMAD_LTU_SOLID_EDGE_XCOOR_M, TX.LTU_SOLID_EDGE_X_COOR) * 3.28083989501, 6)
