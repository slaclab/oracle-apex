create or replace TRIGGER SSRL_RSW_FORM_BIUDR
...
if updating then
    ...
    --
    -- If Person Assigned is not the same as Person signing in, nullify the PR Signoff
...
-- Any change of the Section 5 Checkboxes to Null
    ...
    --
    if (:old.S6_OPERATOR_ACK is null and :new.S6_OPERATOR_ACK = 'Y')
    then
         :new.S6_OPERATOR_ACK_DATE        := sysdate;
         :new.S6_OPERATOR_ACK_MODIFIED_BY := l_current_user;
--	 :new.EMAIL_CHK := 'Y';	
--	 :new.email_rule := 16;
    end if;
    --
	...
	,p_s6_operator_ack	=> :new.S6_OPERATOR_ACK
    ...
    --
    -- Form Status = Work Approved (not yet Released) - Email 
    if :new.form_status_id = 2 and
    ...
    --
     -- Form Status = CLOSED - Email 
   if :new.form_status_id = 6 and
       :new.form_status_id != :old.form_status_id
    then
	 :new.EMAIL_CHK := 'Y';	
	 :new.email_rule := 23;
    ...
    --
    -- Form Status = DROPPED - Email 
    if :new.form_status_id = 7 and
       :new.form_status_id != :old.form_status_id
    then
	 :new.EMAIL_CHK := 'Y';	
	 :new.email_rule := 24;
