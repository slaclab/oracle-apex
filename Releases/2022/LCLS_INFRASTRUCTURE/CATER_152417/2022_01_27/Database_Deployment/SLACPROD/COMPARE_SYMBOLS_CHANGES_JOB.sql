BEGIN
  SYS.DBMS_SCHEDULER.DROP_JOB
    (job_name  => 'LCLS_INFRASTRUCTURE.COMPARE_SYMBOLS_CHANGES_JOB');
END;
/
...
BEGIN
  SYS.DBMS_SCHEDULER.CREATE_JOB
    (
      ...
      ,job_action      => '"LCLS_INFRASTRUCTURE"."COMPARE_SYMBOLS_COLS_CHANGE"'
      ,comments        => 'Job runs everyday at 5pm to alert users of any Linacz_m and Engineering name changes.'
      ,auto_drop       => FALSE
      ,enabled         => TRUE);

  SYS.DBMS_SCHEDULER.SET_ATTRIBUTE
    ( name      => 'LCLS_INFRASTRUCTURE.COMPARE_SYMBOLS_CHANGES_JOB'
...
BEGIN
 DBMS_SCHEDULER.ADD_JOB_EMAIL_NOTIFICATION (
  job_name   =>  'COMPARE_SYMBOLS_CHANGES_JOB',
  recipients =>  'poonam@slac.stanford.edu',
  sender     =>  'COMPARE_SYMBOLS_CHANGES_JOB@slac.stanford.edu',
...
--exec dbms_scheduler.STOP_JOB('COMPARE_SYMBOLS_CHANGES_JOB');
--exec dbms_scheduler.DISABLE('COMPARE_SYMBOLS_CHANGES_JOB');

-- To enable it again
--exec dbms_scheduler.ENABLE('COMPARE_SYMBOLS_CHANGES_JOB');
--exec dbms_scheduler.RUN_JOB('COMPARE_SYMBOLS_CHANGES_JOB');
