create or replace TRIGGER "LCLS_INFRASTRUCTURE"."MAGNET_STRING_BIUDR" 
...
DECLARE
  ...
  l_slc_micro_name		lcls_elements.slc_micro_name%TYPE := NULL;
...
apps_util.utl.log_add(p_appl_id => 2, p_trans_id => null, p_message_type_id => 1 ,p_text => c_proc || ' :old.SAFETY_SYSTEM= '||:old.SAFETY_SYSTEM);
  ...
  IF INSERTING THEN
    ...
    --
    IF :new.SAFETY_SYSTEM is null THEN
  	:new.SAFETY_BCS := 0;
  	:new.SAFETY_MPS := 0;
  	:new.SAFETY_PPS := 0;
	:new.SAFETY_SYSTEM := 0;
   ...
   exception
	WHEN NO_DATA_FOUND THEN 
         ERRMSG := 'ERROR: Polynomial record not found for Barcode='||:new.barcode;
...
IF INSERTING OR UPDATING THEN
  begin
   IF (nvl(:new.SAFETY_SYSTEM,99) != nvl(:old.SAFETY_SYSTEM,99)) then
   ...
   END IF;      -- IF (nvl(:new.SAFETY_SYSTEM,99) != nvl(:old.SAFETY_SYSTEM,99)) 
  end;
  --
    ...
    --
    IF nvl(:new.UNIT_DISPLAY_OVERRIDE,'N') = 'N' THEN
      begin
          select ps_config
	  into l_ps_config
	  from magnet_polynomial
	...
	:new.UNIT_DISPLAY := 'mgnt_unit2';
      END IF;
    END IF; -- :new.UNIT_DISPLAY_OVERRIDE = 'N'
    --
    -- set HW_AREA value --------------------
    --
    begin
      select element, EPICS_CHANNEL_ACCESS_NAME, ENGINEERING_NAME, SLC_MICRO_NAME
	  into l_element, l_epics_channel_access_name, l_engineering_name, l_slc_micro_name
    ...
    --
-- Poonam 10/3/2022 - Cater #141726 Task #56
    IF l_slc_micro_name IS NOT NULL THEN
	IF (nvl(:new.PSC_TYPE,'x') != 'SLC') THEN
	   :new.PSC_TYPE := 'SLC';
	END IF;
    END IF;  -- l_slc_micro_name IS NOT NULL
    --
    ...
    --
     IF nvl(:new.HW_DISPLAY_OVERRIDE,'N') = 'N' THEN
--        (nvl(:old.HW_DISPLAY_OVERRIDE,'N') != nvl(:new.HW_DISPLAY_OVERRIDE,'N')) THEN
...
--	END IF;    -- (:new.PSC_TYPE != nvl(:old.PSC_TYPE,'x'))
	...
	:new.HW_DISPLAY := '';
     ...
     END IF; -- :new.HW_DISPLAY_OVERRIDE = 'N'
    --
    -- set LIMIT_DISPLAY value --------------------
    -- Poonam 7/20/2022 - Modified the below code from HW_DISPLAY check to PSC_TYPE check
    --
    IF (nvl(:new.PSC_TYPE,'x') != nvl(:old.PSC_TYPE,'x')) THEN     
     IF :new.PSC_TYPE in ('EPSC','NONE') THEN
