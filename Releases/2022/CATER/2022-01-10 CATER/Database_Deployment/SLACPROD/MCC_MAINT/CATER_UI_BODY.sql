...
--------------------------------------------------------
  ...
  CREATE OR REPLACE PACKAGE BODY "MCC_MAINT"."CATER_UI" as

-- Poonam - 1/10/2022 - Changed Day+time to Month/date+time when PMM > 1 day, as weeks long PMMs were creating confusion with just Day.
...
-- Poonam - 1/10/2022 - Show month/date+time when PMM > 1 day
     IF l_print_day = 1 THEN
       l_note_time := to_char(note_rec.note_time,'MM/DD HH24:MI');
     ELSE
...
-- Poonam - 1/10/2022 - Changed Day+time to Month/date+time when PMM > 1 day
     IF l_print_day = 1 THEN
       l_survey_time := to_char(cater_rec.survey_time,'MM/DD HH24:MI');
       l_sched_start_time := to_char(cater_rec.sched_start_time,'MM/DD HH24:MI');
       l_search_time := to_char(cater_rec.search_time,'MM/DD HH24:MI');
       l_sched_end_time := to_char(cater_rec.sched_end_time,'MM/DD HH24:MI');
     ELSE
